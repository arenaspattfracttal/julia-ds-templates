...
import {
  ...
  ChevronDown, Trash2, Pencil, Eye, EllipsisVertical,
} from "lucide-react"
...
function parseISO(str: string) { return new Date(str) }
function formatDate(date: Date, _fmt: string) { return date.toISOString().split("T")[0] }
...
const FESTIVOS_COLUMNS: ColumnDef<DiaFestivo>[] = [
  {
    accessorKey: "descripcion",
    header: "Descripción",
    cell: ({ row }) => (
      <span className="text-sm font-medium">{row.original.descripcion}</span>
    ),
  },
  {
    accessorKey: "fecha",
    header: "Fecha",
    cell: ({ row }) => (
      <span className="text-sm tabular-nums text-muted-foreground">{row.original.fecha}</span>
    ),
  },
  {
    accessorKey: "diaLaboral",
    header: "Día laboral",
    cell: ({ row }) => {
      const v = row.original.diaLaboral
      return <Badge variant={v ? "success" : "destructive"} size="sm">{v ? "Sí" : "No"}</Badge>
    },
  },
  {
    ...
    header: "Recurrente",
    cell: ({ row }) => {
      const v = row.original.recurrente
      return <Badge variant={v ? "success" : "destructive"} size="sm">{v ? "Sí" : "No"}</Badge>
    },
  },
]
    ...
    <>
      ...
      <div className="flex-1 overflow-hidden flex flex-col min-h-0">
        <DataTable
          ...
          rowDensity
          onAdd={openCreate}
          addLabel={isCompact ? undefined : "Nuevo festivo"}
          onRefresh={() => setRows(FESTIVOS_INICIAL)}
          rowActions={(row) => (
            <DropdownMenu>
              <Tooltip>
                <TooltipTrigger asChild>
